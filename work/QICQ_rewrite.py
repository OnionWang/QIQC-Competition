...
class Attention(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        eij = self.linear(x.contiguous().view(-1, feature_dim))
...
if __name__ == '__main__':
    ...
    for n in range(num):
        ...
        for i, (train_index, val_index) in enumerate(kfold):
            ...
            model.cuda()
            print(f'Fold {i + 1}') 
            y_pred, test_pred = train_model(model, X_fold, y_fold, val_X_fold, val_y_fold, X_test, param['sentence_len'], batch_size, n_epochs, False)
